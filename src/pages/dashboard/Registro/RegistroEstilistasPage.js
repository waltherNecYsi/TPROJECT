...
import {
  EstilistasTableRow,
  EstilistasTableToolbar,
  EstilistasTableButtom,
  EstilistasTableEdit,
...
export default function RegistroEstilistasPage() {
  ...
  const modal1Request = async (formData, closeModal) => {
    try {
      const response = await axios.post(`/api/estilistas`, {
        ...formData,
      });
  ...
  const handleShowEditModal = async (id) => {
    try {
      const response = await axios.get(`/api/estilistas/${id}`);
      return response.data;
    } catch (error) {
  ...
  if (filterName) {
    inputData = inputData.filter(
      (invoice) =>
        ...
        invoice.ane_nom.toLowerCase().indexOf(filterName.toLowerCase()) !== -1
    );
  }
