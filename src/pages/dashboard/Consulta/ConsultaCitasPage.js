...
import {
  ...
  CliTableButtom,
  CliTableEdit,
} from "../../../sections/@dashboard/Consultas/Citas";
...
export default function RegistroClientesPage() {
  ...
  const handleShowEditModal = async (id) => {
    const response = await axios.get(`/api/cliente/${id.ClienteID}`);
    return response.data;
  };
  ...
  const fetchDataFromAPI = useCallback(async () => {
    try {
      const response = await axios.get(`/api/cita_dominio`, {
        params: {
          page: page + 1,
