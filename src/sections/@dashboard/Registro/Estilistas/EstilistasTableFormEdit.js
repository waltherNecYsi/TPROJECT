...
export default function EstilistasTableFormEdit({
  inputs,
  request,
  closeModal,
  fetchDataFromAPI,
  ...
  useEffect(() => {
    if (rowData) {
      const fetchData = async () => {
        try {
          const ubigeoResponse = await axios.post(`/api/ubigeo`);
          
          const selectedOption = ubigeoResponse.data.find(
  ...
  const busquedaSunat = async () => {
    ...
    try {
      ...
      if (documentoValue.id === '1') {
        ...
        try {
          const busquedaubisunat = await axios.post(`/api/busquedaubigeo`, {
  ...
  const onSubmit = async () => {
    ...
    try {
      const response = await axios.put(`/api/cliente/${rowData.ane_id}`, DataSubmit);
      closeModal();
      fetchDataFromAPI();
    } catch (error) {
      console.error(error);
...
EstilistasTableFormEdit.propTypes = {
